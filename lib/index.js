...
`)
  ...
  async updateStatus (message, pctProgress) {
    ...
    await fs.writeFile(
      // it is possible that the manifest has invalid JSON and is unparseable,
  ...
  async validateManifest () {
    ...
    if (!buildGraph && !runServer) {
      errors.push('At least one of `buildGraph` or `runServer` must be set to true')
    }

    // if build is set to true, then routerFolderDownloads needs to be defined
    if (buildGraph && (!routerFolderDownloads || !routerFolderDownloads.length)) {
      errors.push('`routerFolderDownloads` must be populated for graph build (need inputs)')
      console.log(routerFolderDownloads)
    }
  ...
  async run () {
    ...
    // add tasks to download files to the router folder
    if (
      this.manifest.routerFolderDownloads &&
        this.manifest.routerFolderDownloads.length > 0
    ) {
      this.manifest.routerFolderDownloads.forEach(url => {
    ...
    if (!this.manifest.buildGraph && this.manifest.runServer) {
      // this.manifest says to run the server without building a graph.
    ...
    this.status.totalFilesToDownload = this.downloadTasks.length
    await this.updateStatus(`Downloading ${this.status.totalFilesToDownload} files...`, 10)
    await Promise.all(
      this.downloadTasks.map(download => this.makeDownloadTask(download))
    )
